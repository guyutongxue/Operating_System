...
found:
  ...
  // Kstack should mapped to the kernel pagetable per proc.
  uint64 ks_va = KSTACK((int) (p - proc));
  mappages(p->kpagetable, ks_va, PGSIZE, (uint64)p->kstack_pa, PTE_R | PTE_W);
  p->kstack = ks_va;
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    if (u2kvmcopy(p->pagetable, p->kpagetable, p->sz, p->sz + n) < 0) {
      return -1;
    }
  } else if(n < 0){
...
fork(void)
{
  ...
  // Copy new process's user pgtbl to kernel pgtbl.
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        // Load kernel pagetable per proc before swtch.
